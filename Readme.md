...
| KPI 항목            | 설명                                      |
| ----------------- | --------------------------------------- |
| ⏱ **알림 반영 속도 향상** | 실시간 시스템 반영 속도를 수 초 이내로 줄여 사용자 응답성 개선    |
| 🎯 **관제 효율성 향상**  | 수동 처리 절차를 자동화하여 관제 인력의 업무 효율 및 반응 시간 개선 |
| 😊 **사용자 만족도 향상** | 사용자 피드백 기반 개선을 통해 기능·UI 품질 만족도 수치로 측정   |
| 🔧 **플랫폼 확장성 확보** | 기존 기능 기반으로 차기 프로젝트 연계·확장이 가능한 구조를 기획·완료 |
---
...
| KPI 항목                | 설명                                       |
| --------------------- | ---------------------------------------- |
| 🔔 **실시간 알림 자동화 도입률** | 기존 수동/지연성 있는 업무 프로세스를 자동화된 실시간 알림 구조로 전환 |
| ✅ **내부 개선 요청 처리율**    | 사내 건의사항 등 공식 채널을 통한 요청을 시스템 도입으로 반영한 실적  |
| 🛠 **시스템 신뢰성 및 복구율**  | 예외 상황 (토큰 만료, 중복 발송 등) 자동 대응 구조를 갖춘 정도   |
...
| 🔔 **실시간 알림 자동화 도입률**     | 기존 수동/지연성 있는 업무 프로세스를 자동화된 실시간 알림 구조로 전환   |
| ✅ **내부 개선 요청 처리율**        | 사내 건의사항 등 공식 채널을 통한 요청을 시스템 도입으로 반영한 실적    |
| 📦 **업무 시스템 자동화율**        | 수기로 처리되던 관리 항목을 전산화하여 누락·오류를 줄이고 생산성 향상    |
| 🧰 **장비 관리 정확도 향상**       | 정기 점검, 상태 이력 등을 통해 장비 누락 관리율 감소            |
| 🔄 **실시간 대응 체계 구축**       | PLC 및 설비 상태를 실시간 감지하고 알림으로 전파하는 체계의 신속성 확보 |
| 🧹 **기존 시스템 안정성 및 유지보수율** | 오래된 코드·구조 개선을 통해 운영 유지비용 절감 및 안정성 확보       |
| 💾 **데이터 수집 자동화율**        | 외부 데이터를 자동 수집·DB 저장하여 반복 업무를 자동화한 비율       |
...
|                  | HTML / CSS                       | ✅ |
|                  | T-SQL                            | ✅ |
| **백엔드 프레임워크** | .NET Core 3.1 / .NET 6.0           | ✅ |
|                  | SignalR                          | ✅ |
| **프론트엔드 라이브러리** | Leaflet.js                        | ✅ |
|                  | jQuery                           | ✅ |
|                  | Bootstrap                        | ✅ |
|                  | AdminLTE 4                       | ✅ |
| **DB**            | Microsoft SQL Server             | ✅ |
|                  | MariaDB                          | ✅ |
| **API 연동**      | Kakao BizTalk API                | ✅ |
|                  | 공공기관 내부 API (철도공사)      | ✅ |
| **배포 / 자동화** | IIS 수동 배포                    | ✅ |
|                  | BAT 스크립트 자동화              | ✅ |
|                  | Windows 작업 스케줄러            | ✅ |
